  ...
  .done( function() {
    ...
    navigationItem.each(function(index, elem) {
      ...
      element = document.createElement('i');
        element.classList.add('fa-solid', 'fa-arrow-right-to-bracket');
        link.appendChild(element);
      if(exist[0] == 0){
        exist[0]++;
        document.getElementsByClassName('headline')[0].appendChild(link);
      }
    });
    ...
    function doTween(target) {
      ...
      timeline
        ...
        link = document.createElement('a');
        link.href = `pagina${$(target).index()}.html`;
        element = document.createElement('i');
        element.classList.add('fa-solid', 'fa-arrow-right-to-bracket');
        link.appendChild(element);
        if(exist[$(target).index()] == 0){
          exist[$(target).index()]++;
          document.getElementsByClassName('headline')[$(target).index()].appendChild(link);
        }
    }
